...
export class FolderService {
  ...
  async getFolderById(folderId: string): Promise<Folder | null> {
    const folderDoc = await getDoc(doc(this.firestore, 'folders', folderId));
    if (folderDoc.exists()) {
  ...
  async getFolderItemIds(folderId: string, itemType?: ItemType): Promise<string[]> {
    const relationsRef = collection(this.firestore, 'folder_items');
  ...
  async addItemToFolder(
    ...
    const folderItemsRef = collection(this.firestore, 'folder_items');
